...
const iota_lib_js_1 = __importDefault(require("iota.lib.js"));
...
class MamProvider {
     ...
     */
    constructor(provider, seed = utils.generateKey(), security = 2, depth = 4, mwm = 9, nextRoot = null) {
        this.iota = new iota_lib_js_1.default({ provider: provider });
        this.depth = depth;
        this.mwm = mwm;
        this.cache = new Map();
    ...
    /**
      ...
      */
    async publish(packet, sideKey = null) {
        ...
        const trytes = this.iota.utils.toTrytes(JSON.stringify(packet, (key, value) => {
     ...
     */
    async _fetch(root, sideKey, callback) {
        var dataArray = [];
        if (this.cache.has(root)) {
            var cached = this.cache.get(root);
            cached.data.forEach(function (element) {
                callback(element);
            });
            return cached.nextRoot;
        }
        const getTrytes = (data) => {
            const parsedDataFromTrytes = JSON.parse(this.iota.utils.fromTrytes(data.toString()), (key, value) => {
...
exports.MamProvider = MamProvider;
//# sourceMappingURL=MamProvider.js.map